...
class QRMask
{
    ...
    {
        $this->runLength = array_fill(0, QRConfig::QRSpec_WIDTH_MAX + 1, 0);
    }
    ...
    public function makeMaskNo($maskNo, $width, $s, &$d, $maskGenOnly = false)
    {
        ...
        if (QRConfig::QR_CACHEABLE) {
            if (file_exists($fileName)) {
                ...
                if (!file_exists(QRConfig::QR_CACHE_DIR . 'mask_' . $maskNo)) {
                    mkdir(QRConfig::QR_CACHE_DIR . 'mask_' . $maskNo);
                }
    ...
    public function calcN1N3($length)
    {
        ...
        for ($i = 0; $i < $length; ++$i) {
            if ($this->runLength[$i] >= 5) {
                $demerit += (QRConfig::N1 + ($this->runLength[$i] - 5));
            }
    ...
    public function mask($width, $frame, $level)
    {
        ...
        if (0 !== QRConfig::QR_FIND_FROM_RANDOM) {
        ...
        foreach ($checked_masks as $i) {
            ...
            $demerit = (int) ((int) (abs($blacks - 50) / 5) * QRConfig::N4);
