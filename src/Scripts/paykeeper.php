...
/*file_put_contents(INNER_PATH . 'scripts/paykeeper.txt', '
' . file_get_contents('php://input'), FILE_APPEND);*/

/* данные от PayKeeper */
$transactionId = (int) $_REQUEST['orderid'];
        ...
        if ($transactionData['id'] > 0 && $transactionData['conversation_message_id'] === $conversationMessageData['id']) {
            ...
            if ($paymentTypeData['pk_type'] === '1') {
                ...
                foreach ($fieldsToHash as $fieldToHash) {
                    $hashStr .= $fieldToHash === 'sum' ? number_format(
                        $_REQUEST[$fieldToHash],
                        2,
                        '.',
                        '',
                    ) : $_REQUEST[$fieldToHash];
                }
                $hashStr .= DataHelper::escapeOutput($projectData['paykeeper_secret']);

                $hashStrHashed = md5($hashStr);

                if ($hashStrHashed === $_REQUEST['key']) {
                    ...
                    if ($transactionData['verified'] === '0') {
                        ...
                        $comissionPercent = 1.95 + 4;

                        if ($_REQUEST['ps_id'] === 'SBP') {
                            // если СБП, то 0.7% за СБП + 4% за самозанятого
                            $comissionPercent = 0.7 + 4;
                        }
                        $comissionValue = (float) $_REQUEST['sum'] / 100 * $comissionPercent;
                    ...
                    if ($transactionDone) {
                        ...
                        $result = true;
                        $content = 'OK ' . md5($transactionId . DataHelper::escapeOutput($projectData['paykeeper_secret']));
                    } else {
                        $content = 'PayKeeper прислала непонятный тип события по транзакции.';
                    }
                } else {
                    $content = 'Неверная подпись запроса.';
                }
            } else {
                $content = 'У данной транзакции установлен тип оплаты отличный от PayKeeper.';
            }
        } else {
